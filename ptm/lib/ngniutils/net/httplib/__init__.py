...
from ngniutils import NOT_SET, Base
import socket
import exc as _exc
import sys
import types

class HTTPResponseWrapper(Base):
...
class HTTPSConnection(_HTTPSConnection):
	...
	def __init__(self, host, port=None, strict=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT, source_address=None, response_wrapper = NOT_SET):
		_HTTPSConnection.__init__(self, host, port, strict = strict, timeout = timeout, source_address = source_address)
		if response_wrapper is not NOT_SET:
			self.response_wrapper = response_wrapper
