...
void print_help() {
	printf("impd4e - a probe based on lib_pcap which applies hash-based selection and exports packetIDs via IPFIX to a collector"
			"\n"
			""
...
void flush_interfaces() {
	...
	for (j=0; j<options.number_interfaces; j++ ) {
		if ( timevaldiff( &(pcap_devices[j].last_export_time), &now) > options.export_interval * 1000 ) {
...
void parse_cmdline(options_t *options, int argc, char **argv) {
	...
	int c;
	char par[] = "i:I:o:r:t:f:m:M:hs:F:c:P:C:v";
	char *endptr;
	errno = 0;
...
//		mlogf(ALWAYS, "Device %s has IP %s \n", options->if_names[i], htoa(pcap_devices[i].IPv4address));
		...
		case DLT_EN10MB:
			pcap_devices[i].offset[L_NET] = 14;
			mlogf(INFO, "dltype: DLT_EN10M \n");
			break;
		case DLT_ATM_RFC1483:
			pcap_devices[i].offset[L_NET] = 8;
			mlogf(INFO, "dltype: DLT_ATM_RFC1483\n");
			break;
		case DLT_LINUX_SLL:
			pcap_devices[i].offset[L_NET] = 16;
			mlogf(INFO, "dltype: DLT_LINUX_SLL\n");
			break;
		case DLT_RAW:
			pcap_devices[i].offset[L_NET] = 0;
			mlogf(INFO, "dltype: DLT_RAW\n");
			break;
		default:
			mlogf(ALWAYS,
					"Link Type (%d) not supported - default to DLT_RAW \n",
					pcap_devices[i].link_type);
			pcap_devices[i].offset[L_NET] = 0;
			break;
		}
		...
		if (options->bpf) {
			if (pcap_compile(pcap_devices[i].pcap_handle, &fp, options->bpf, 0,
					pcap_devices[i].IPv4address) == -1) {
				mlogf(ALWAYS, "Couldn't parse filter %s: %s\n", options->bpf,
						pcap_geterr(pcap_devices[i].pcap_handle));
				continue;
			}
			if (pcap_setfilter(pcap_devices[i].pcap_handle, &fp) == -1) {
...
void open_ipfix_export(pcap_dev_t *pcap_devices, options_t *options) {
	...
	for (i = 0; i < (options->number_interfaces); i++) {
		...
		case TS_TTL_PROTO_ID:
			if (ipfix_make_template(pcap_devices[i].ipfixhandle,
					&(pcap_devices[i].ipfixtemplate), export_fields_ts_ttl_proto, 6) < 0) {
				mlogf(ALWAYS, "ipfix_make_template_ts_ttl_proto_id() failed: %s\n", strerror(errno));
				exit(1);
			}
...
void handle_packet(u_char *user_args, const struct pcap_pkthdr *header,
	...
	if ((pcap_device->options->sel_range_min < hash_result)
		...
		switch (pcap_device->options->templateID) {
			case MINT_ID:
			{
				timestamp = (unsigned long long) header->ts.tv_sec * 1000000ULL + header->ts.tv_usec;


				void *fields[] = {&timestamp, &hash_result, &ttl };
				uint16_t lengths[] = { 8, 4, 1 };

				if ( ipfix_export_array( pcap_device->ipfixhandle, pcap_device->ipfixtemplate, 3, fields, lengths ) <0 ) {
			...
			case TS_TTL_PROTO_ID:
			{
				...
				if ( ipfix_export_array( pcap_device->ipfixhandle, pcap_device->ipfixtemplate, 6, fields, lengths ) <0 ) {
						            fprintf( stderr, "ipfix_export() failed: %s\n",
						                     strerror(errno) );
						            exit(1);
						  }
...
void run_pcap_loop(pcap_dev_t *pcap_devices, options_t *options) {
	...
	if (options->number_interfaces == 1) {
		...
		while(cnt != 0 && status >= 0) {
		    ...
		    if (err < 0) {
		       	if (errno == EINTR)  {
		       		...
		       		if (alarmmm == 1) {  /* did we got a timer alarm? */
				       	...
				       	mlogf(ALWAYS, "select interrupted by interrupt != alarm");
						continue;
		       		}
			...
			while(cnt != 0 && status >= 0) {
				for(i=0, n=0; i<options->number_interfaces; i++) {
						status = pcap_dispatch(pcap_devices[i].pcap_handle, (cnt > 0 && cnt < 10 ? cnt : 10), handle_packet, (u_char*) &pcap_devices[i]);
						if (status < 0) {
							errdev = i;
							break;
						}
		...
		mlogf(ALWAYS, "Error DeviceNo %d %s: pcap_loop: %s\n", errdev,
					options->if_names[errdev], pcap_geterr(pcap_devices[errdev].pcap_handle));
