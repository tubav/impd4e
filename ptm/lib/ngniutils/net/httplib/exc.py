...
class HTTPErrorType(type):
	...
	def get_error_class(cls, status):
		...
		try:
			...
			errorcls = cls(name, (HTTPError, ), {"__init__": cls._make_init(status)})
	...
	def __call__(self, *args, **kw):
		if self is HTTPError and args:
			self = self.get_error_class(args[0])
			args = args[1:]
	...
	def _make_init(cls, status):
		def __init__(self, msg = None, reason = None, *args, **kw):
			super(self.__class__, self).__init__(status, reason, msg, *args, **kw)
		return __init__
