...
void packet_pcap_cb(u_char *user_args, const struct pcap_pkthdr *header,
	...
	packet_ptr = packet + g_options.offset;
	...
	copiedbytes = g_options.selection_function(packet_ptr, header->caplen - g_options.offset,
			if_device->outbuffer, if_device->outbufferLength,
			if_device->offset, layers);

	ttl = getTTL(packet_ptr, header->caplen- g_options.offset, if_device->offset[L_NET],
			layers[L_NET]);
	...
	if ((g_options.sel_range_min <= hash_result) && (g_options.sel_range_max
			>= hash_result)) {
		if_device->export_packet_count++;
		if_device->sampling_size++;
		...
		case TS_ID: {
			...
			index
					+= set_value(&fields[index], &lengths[index], &hash_result,
							4);
			break;
		}
		...
		case TS_TTL_PROTO_ID: {
			if (layers[L_NET] == N_IP) {
				length = ntohs(
						*((uint16_t*) (&packet_ptr[if_device->offset[L_NET] + 2])));
			} else if (layers[L_NET] == N_IP6) {
				length = ntohs(
						*((uint16_t*) (&packet_ptr[if_device->offset[L_NET] + 4])));
			} else {
				LOGGER_fatal("cannot parse packet length");
				length = 0;
			}
			...
			index += set_value(&fields[index], &lengths[index],
					&layers[L_TRANS], 1);
			index += set_value(&fields[index], &lengths[index], &layers[L_NET],
					1);
			break;
		}
		...
		case TS_TTL_PROTO_IP_ID: {
			...
			if (N_IP == layers[L_NET]) {
				index += set_value(&fields[index], &lengths[index],
						(uint32_t*) &packet_ptr[if_device->offset[L_NET] + 12], 4);
			} else {
			...
			case T_UDP:
				...
				port
						= ntohs(
								*((uint16_t*) &packet_ptr[if_device->offset[L_TRANS]]));
				break;
			default:
				port = 0;
			}
			...
			if (N_IP == layers[L_NET]) {
				index += set_value(&fields[index], &lengths[index],
						(uint32_t*) &packet_ptr[if_device->offset[L_NET] + 16], 4);
			} else {
				index += set_value(&fields[index], &lengths[index], &ipa, 4);
			}
			switch (layers[L_TRANS]) {
			...
			case T_UDP:
				//case T_SCTP:
				port = ntohs(
						*((uint16_t*) &packet_ptr[if_device->offset[L_TRANS] + 2]));
				break;
			default:
				port = 0;
			}
		...
		// send ipfix packet
		if (0 > ipfix_export_array(if_device->ipfixhandle, template, size,
				fields, lengths)) {
			LOGGER_fatal("ipfix_export() failed: %s", strerror(errno));
			exit(1);
		}
